...
window.renderDock = function () {
  ...
  if (!State.CONFIG.dock_links || !Array.isArray(State.CONFIG.dock_links))
    return;
...
function bindGlobals() {
  ...
  closeButtons.forEach((btn) => {
    ...
    btn.onclick = function () {
      window.ModalManager.closeTop(false);
    };
  });
  ...
  if (noteHelpClose)
    noteHelpClose.onclick = function () {
      window.ModalManager.close("note-help-modal");
    };

  const noteHelpBtn = document.getElementById("note-help-btn");
  if (noteHelpBtn)
    noteHelpBtn.onclick = function () {
      window.ModalManager.toggle("note-help-modal");
    };
  ...
  if (saveBtn)
    saveBtn.onclick = function () {
      window.SettingsUI.saveConfigFromUI();
    };

  const headerSaveBtn = document.getElementById("header-save-btn");
  if (headerSaveBtn)
    headerSaveBtn.onclick = function () {
      window.SettingsUI.saveConfigFromUI();
    };
  ...
  if (dlNoteBtn)
    dlNoteBtn.onclick = function () {
      window.Notes.downloadNotes();
    };
  ...
  if (pomoPause)
    pomoPause.onclick = function () {
      window.startPomodoro && window.Pomodoro
        ? window.Pomodoro.togglePause()
        : null;
    };

  const pomoStop = document.getElementById("pomodoro-stop");
  if (pomoStop)
    pomoStop.onclick = function () {
      window.Pomodoro ? window.Pomodoro.stop() : null;
    };

  const pomoAck = document.getElementById("pomo-dismiss-btn");
  if (pomoAck)
    pomoAck.onclick = function () {
      window.Pomodoro ? window.Pomodoro.acknowledgeComplete() : null;
    };

  // 9. Notes Discovery Hint Dismiss
  const discoveryDismiss = document.getElementById("notes-discovery-dismiss");
  if (discoveryDismiss)
    discoveryDismiss.onclick = function () {
      localStorage.setItem("VINLAND_NOTES_HINT_DISMISSED", "1");
...
async function boot() {
  ...
  if (Clock) Clock.init();
  if (CalloutModal) CalloutModal.init();
